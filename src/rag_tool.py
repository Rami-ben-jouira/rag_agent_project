...
def detect_language(text: str) -> str:
    ...
    # Mots français courants (sans accents) - EXPANDED
    french_keywords = [
        'je', 'suis', 'mon', 'ma', 'mes', 'le', 'la', 'les', 'de', 'du', 'des', 
        'que', 'quoi', 'quelle', 'comment', 'pourquoi', 'symptome', 'symptôme',
        'maladie', 'traitement', 'cause', 'avoir', 'souffrir', 'malade', 
        'j ai', 'quels', 'qu est', 'fatigue', 'fatigué', 'mal', 'douleur',
        'tête', 'fièvre', 'toux', 'nez', 'gorge', 'ventre', 'dos', 'maux',
        'souffre', 'souffrant', 'malades', 'maladies', 'symptômes'
    ]
    # Mots anglais courants
    english_keywords = [
        'i', 'my', 'the', 'a', 'is', 'are', 'have', 'disease', 'symptom', 
        'treatment', 'cause', 'what', 'which', 'how', 'why', 'affected', 
        'suffer', 'pain', 'headache', 'fever', 'cough', 'throat', 'stomach'
    ]
    
    # Check for French words (word boundaries to avoid false matches)
    import re
    french_count = 0
    ...
    # If any French keyword found, prioritize French
    if french_count > 0:
        return 'fr'
    elif english_count > 0:
        return 'en'
    else:
        # Default to English if no keywords found
        return 'en'
...
Analyze the patient question below. If it contains French words like "je", "suis", "fatigue", "maladie", "symptôme", "j'ai", "mal", "tête", "douleur" OR French accents (é, è, ê, à, ç, etc.), you MUST respond ENTIRELY IN FRENCH.

**IF THE QUESTION IS IN FRENCH (contains "je", "suis", "fatigue", etc.):**
- Respond ENTIRELY IN FRENCH ONLY - NO EXCEPTIONS
...
**IMPORTANT: If the question contains "je", "suis", "fatigue" or any French words, respond in FRENCH. Otherwise, respond in ENGLISH.**
...
Medical Explanation:"""
    ...
    def _run(self, query: str) -> str:
        ...
        try:
            ...
            print(f"[LANGUAGE DETECTION] Detected language: {lang} for query: {query[:50]}...")
            
            # Enhance query with language hint for better detection
            if lang == 'fr':
                enhanced_query = f"[LANGUE: FRANÇAIS] {query}"
            else:
                enhanced_query = f"[LANGUAGE: ENGLISH] {query}"
